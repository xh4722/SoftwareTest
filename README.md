...
有效的动态测试需要知道关于软件行为的一些定义（输入A输出B或或执行C得到D），接下来要开始定义测试用例。**测试用例**是指进行测试时使用的特定输入，以及测试软件的过程步骤。
### 通过性测试和失效性测试
- **通过性测试（test-to-pass）**：确认软件至少能做什么，而不会考验其能力。在设计和执行测试用例时，总是首先进行通过性测试，
- **失效性测试（test-to-fail）**：纯粹为了破坏软件而设计和执行的测试用例。
### 等价类划分
- **等价类划分**是指分步骤地把海量（无限）的测试用例集减得很小，但过程同样有效。
- **一个等价类或等价类划分**是指测试相同目标或者暴露相同软件缺陷的一组测试用例。
